...
* Quantum step *Q*
  ...
  * CQP had `*=` for measure, which looked like an assignment, so (at Guillaume Poly's suggestion) I changed it to `=?`. And then I changed it to `-/-` to try to make it look like the meter symbol that's used in quantum circuit diagrams.   
